...
unsafe extern "C" fn g_fork_detect_once_bss_get() -> *mut CRYPTO_once_t {
    return &mut g_fork_detect_once;
}
static mut g_fork_detect_once: CRYPTO_once_t = 0 as libc::c_int;
unsafe extern "C" fn g_fork_detect_lock_bss_get() -> *mut CRYPTO_STATIC_MUTEX {
    return &mut g_fork_detect_lock;
}
static mut g_fork_detect_lock: CRYPTO_STATIC_MUTEX = {
...
unsafe extern "C" fn g_fork_detect_addr_bss_get() -> *mut *mut libc::c_char {
    return &mut g_fork_detect_addr;
}
static mut g_fork_detect_addr: *mut libc::c_char = 0 as *const libc::c_char
    as *mut libc::c_char;
unsafe extern "C" fn g_fork_generation_bss_get() -> *mut uint64_t {
    return &mut g_fork_generation;
}
static mut g_fork_generation: uint64_t = 0;
unsafe extern "C" fn g_ignore_madv_wipeonfork_bss_get() -> *mut libc::c_int {
    return &mut g_ignore_madv_wipeonfork;
}
static mut g_ignore_madv_wipeonfork: libc::c_int = 0;
unsafe extern "C" fn init_fork_detect_madv_wipeonfork(
    mut addr: *mut libc::c_void,
    mut page_size: libc::c_long,
) -> libc::c_int {
    if madvise(addr, page_size as size_t, -(1 as libc::c_int)) == 0 as libc::c_int
        || madvise(addr, page_size as size_t, 18 as libc::c_int) != 0 as libc::c_int
    {
        return 0 as libc::c_int;
    }
    return 1 as libc::c_int;
}
unsafe extern "C" fn init_fork_detect() {
    let mut res: libc::c_int = 0 as libc::c_int;
    let mut addr: *mut libc::c_void = -(1 as libc::c_int) as *mut libc::c_void;
    let mut page_size: libc::c_long = 0 as libc::c_int as libc::c_long;
    if !(*g_ignore_madv_wipeonfork_bss_get() == 1 as libc::c_int) {
        page_size = sysconf(_SC_PAGESIZE as libc::c_int);
        if !(page_size <= 0 as libc::c_int as libc::c_long) {
            addr = mmap(
            ...
            if !(addr == -(1 as libc::c_int) as *mut libc::c_void) {
                if !(init_fork_detect_madv_wipeonfork(addr, page_size)
                    == 0 as libc::c_int)
                {
                    ...
                    let ref mut fresh0 = *g_fork_detect_addr_bss_get();
                    *fresh0 = addr as *mut libc::c_char;
                    *g_fork_generation_bss_get() = 1 as libc::c_int as uint64_t;
                    res = 1 as libc::c_int;
                }
            }
        }
    }
    if res == 0 as libc::c_int && addr != -(1 as libc::c_int) as *mut libc::c_void {
        munmap(addr, page_size as size_t);
...
pub unsafe extern "C" fn CRYPTO_get_fork_generation() -> uint64_t {
    CRYPTO_once(
        ...
        Some(init_fork_detect as unsafe extern "C" fn() -> ()),
    );
    let flag_ptr: *mut libc::c_char = *g_fork_detect_addr_bss_get();
    if flag_ptr.is_null() {
        return 0 as libc::c_int as uint64_t;
    }
    let lock: *mut CRYPTO_STATIC_MUTEX = g_fork_detect_lock_bss_get();
    let generation_ptr: *mut uint64_t = g_fork_generation_bss_get();
    CRYPTO_STATIC_MUTEX_lock_read(lock);
    let mut current_generation: uint64_t = *generation_ptr;
    if *flag_ptr != 0 {
    ...
    if *flag_ptr as libc::c_int == 0 as libc::c_int {
        ::core::ptr::write_volatile(flag_ptr, 1 as libc::c_int as libc::c_char);
        current_generation = current_generation.wrapping_add(1);
        current_generation;
        if current_generation == 0 as libc::c_int as uint64_t {
            current_generation = 1 as libc::c_int as uint64_t;
        }
