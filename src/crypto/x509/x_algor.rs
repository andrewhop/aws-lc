...
unsafe extern "C" {
    ...
    fn EVP_MD_flags(md: *const EVP_MD) -> uint32_t;
...
static mut X509_ALGOR_seq_tt: [ASN1_TEMPLATE; 2] = unsafe {
    [
        {
        ...
        },
        {
            let mut init = ASN1_TEMPLATE_st {
                ...
                field_name: b"parameter\0" as *const u8 as *const libc::c_char,
...
pub unsafe extern "C" fn X509_ALGOR_set0(
    ...
    mut pval: *mut libc::c_void,
) -> libc::c_int {
    if alg.is_null() {
        return 0 as libc::c_int;
    }
    if ptype != -(1 as libc::c_int) {
    ...
    if ptype == -(1 as libc::c_int) {
        if !((*alg).parameter).is_null() {
            ...
            (*alg).parameter = 0 as *mut ASN1_TYPE;
        }
    } else {
...
pub unsafe extern "C" fn X509_ALGOR_set_md(
    ...
    if EVP_MD_flags(md) & 2 as libc::c_int as uint32_t != 0 {
        param_type = -(1 as libc::c_int);
    } else {
        param_type = 5 as libc::c_int;
    }
...
pub unsafe extern "C" fn X509_ALGOR_cmp(
    ...
    if ((*a).parameter).is_null() && ((*b).parameter).is_null() {
        return 0 as libc::c_int;
    }
...
unsafe extern "C" fn run_static_initializers() {
    X509_ALGOR_it = {
        let mut init = ASN1_ITEM_st {
            ...
            templates: X509_ALGOR_seq_tt.as_ptr(),
            tcount: (::core::mem::size_of::<[ASN1_TEMPLATE; 2]>() as libc::c_ulong)
                .wrapping_div(::core::mem::size_of::<ASN1_TEMPLATE>() as libc::c_ulong)
                as libc::c_long,
            funcs: 0 as *const libc::c_void,
            size: ::core::mem::size_of::<X509_ALGOR>() as libc::c_ulong as libc::c_long,
            sname: b"X509_ALGOR\0" as *const u8 as *const libc::c_char,
        };
        init
    };
}
