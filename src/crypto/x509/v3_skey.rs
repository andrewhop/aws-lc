...
pub unsafe extern "C" fn s2i_ASN1_OCTET_STRING(
    ...
    let mut data: *mut uint8_t = x509v3_hex_to_bytes(str, &mut len);
    if data.is_null() {
    ...
    if len > 2147483647 as libc::c_int as size_t {
        ERR_put_error(
            ...
            b"/home/ubuntu/workspace/oss/aws-lc/crypto/x509/v3_skey.c\0" as *const u8
        ...
        if !oct.is_null() {
            ASN1_STRING_set0(oct, data as *mut libc::c_void, len as libc::c_int);
            return oct;
        }
    }
...
unsafe extern "C" fn i2s_ASN1_OCTET_STRING_cb(
    mut method: *const X509V3_EXT_METHOD,
    mut ext: *mut libc::c_void,
) -> *mut libc::c_char {
    return i2s_ASN1_OCTET_STRING(method, ext as *const ASN1_OCTET_STRING);
}
unsafe extern "C" fn s2i_skey_id(
    ...
    let mut oct: *mut ASN1_OCTET_STRING = 0 as *mut ASN1_OCTET_STRING;
    let mut pk: *mut ASN1_BIT_STRING = 0 as *mut ASN1_BIT_STRING;
    let mut pkey_dig: [libc::c_uchar; 64] = [0; 64];
    let mut diglen: libc::c_uint = 0;
    if strcmp(str, b"hash\0" as *const u8 as *const libc::c_char) != 0 {
        return s2i_ASN1_OCTET_STRING(method, ctx, str) as *mut libc::c_void;
    }
    oct = ASN1_OCTET_STRING_new();
    if oct.is_null() {
    ...
    if !ctx.is_null() && (*ctx).flags == 0x1 as libc::c_int {
        return oct as *mut libc::c_void;
    }
    if ctx.is_null() || ((*ctx).subject_req).is_null() && ((*ctx).subject_cert).is_null()
    {
        ERR_put_error(
            20 as libc::c_int,
            0 as libc::c_int,
        ...
        {
            if !(ASN1_OCTET_STRING_set(oct, pkey_dig.as_mut_ptr(), diglen as libc::c_int)
                == 0)
            {
                return oct as *mut libc::c_void;
            }
...
pub static mut v3_skey_id: X509V3_EXT_METHOD = unsafe {
    {
        let mut init = v3_ext_method {
            ext_nid: 82 as libc::c_int,
            ext_flags: 0 as libc::c_int,
            it: &ASN1_OCTET_STRING_it as *const ASN1_ITEM,
            ext_new: None,
            ext_free: None,
            d2i: None,
            i2d: None,
