...
pub static mut v3_ext_ku: X509V3_EXT_METHOD = unsafe {
    {
        let mut init = v3_ext_method {
            ext_nid: 126 as libc::c_int,
            ext_flags: 0 as libc::c_int,
            it: &EXTENDED_KEY_USAGE_it as *const ASN1_ITEM,
            ext_new: None,
            ext_free: None,
            d2i: None,
            i2d: None,
...
static mut EXTENDED_KEY_USAGE_item_tt: ASN1_TEMPLATE = unsafe {
    {
        let mut init = ASN1_TEMPLATE_st {
            flags: ((0x2 as libc::c_int) << 1 as libc::c_int) as uint32_t,
            tag: 0 as libc::c_int,
            offset: 0 as libc::c_int as libc::c_ulong,
            field_name: b"EXTENDED_KEY_USAGE\0" as *const u8 as *const libc::c_char,
...
pub static mut EXTENDED_KEY_USAGE_it: ASN1_ITEM = unsafe {
    {
        let mut init = ASN1_ITEM_st {
            ...
            templates: &EXTENDED_KEY_USAGE_item_tt as *const ASN1_TEMPLATE,
            tcount: 0 as libc::c_int as libc::c_long,
            funcs: 0 as *const libc::c_void,
            size: 0 as libc::c_int as libc::c_long,
            sname: b"EXTENDED_KEY_USAGE\0" as *const u8 as *const libc::c_char,
        };
        init
    }
};
...
unsafe extern "C" fn i2v_EXTENDED_KEY_USAGE(
    ...
    let mut eku: *const EXTENDED_KEY_USAGE = a as *const EXTENDED_KEY_USAGE;
    let mut i: size_t = 0 as libc::c_int as size_t;
    while i < sk_ASN1_OBJECT_num(eku) {
        let mut obj: *const ASN1_OBJECT = sk_ASN1_OBJECT_value(eku, i);
        let mut obj_tmp: [libc::c_char; 80] = [0; 80];
        i2t_ASN1_OBJECT(obj_tmp.as_mut_ptr(), 80 as libc::c_int, obj);
        X509V3_add_value(0 as *const libc::c_char, obj_tmp.as_mut_ptr(), &mut ext_list);
        i = i.wrapping_add(1);
        i;
    }
    return ext_list;
...
unsafe extern "C" fn v2i_EXTENDED_KEY_USAGE(
    ...
    let mut extku: *mut EXTENDED_KEY_USAGE = sk_ASN1_OBJECT_new_null();
    if extku.is_null() {
    ...
    while i < sk_CONF_VALUE_num(nval) {
        ...
        let mut extval: *const libc::c_char = 0 as *const libc::c_char;
        if !((*val).value).is_null() {
            extval = (*val).value;
        } else {
            extval = (*val).name;
        }
        let mut obj: *mut ASN1_OBJECT = OBJ_txt2obj(extval, 0 as libc::c_int);
        if obj.is_null() || sk_ASN1_OBJECT_push(extku, obj) == 0 {
            ...
            ERR_put_error(
                ...
                0 as libc::c_int,
                129 as libc::c_int,
                b"/home/ubuntu/workspace/oss/aws-lc/crypto/x509/v3_extku.c\0"
    ...
    return extku as *mut libc::c_void;
}
