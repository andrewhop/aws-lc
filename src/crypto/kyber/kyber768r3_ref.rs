    ...
    while i < (256 as libc::c_int / 4 as libc::c_int) as libc::c_uint {
        pqcrystals_kyber768_ref_basemul(
            ...
            &*((*b).coeffs)
                .as_ptr()
                .offset((4 as libc::c_int as libc::c_uint).wrapping_mul(i) as isize),
            pqcrystals_kyber768_ref_zetas[(64 as libc::c_int as libc::c_uint)
...
pub unsafe extern "C" fn pqcrystals_kyber768_ref_poly_tomont(mut r: *mut poly) {
    ...
    while i < 256 as libc::c_int as libc::c_uint {
        (*r)
            .coeffs[i
            as usize] = pqcrystals_kyber768_ref_montgomery_reduce(
...
pub unsafe extern "C" fn pqcrystals_kyber768_ref_poly_reduce(mut r: *mut poly) {
    ...
    while i < 256 as libc::c_int as libc::c_uint {
        (*r)
            .coeffs[i
            as usize] = pqcrystals_kyber768_ref_barrett_reduce((*r).coeffs[i as usize]);
        i = i.wrapping_add(1);
        i;
    }
}
...
pub unsafe extern "C" fn pqcrystals_kyber768_ref_polyvec_basemul_acc_montgomery(
    ...
    while i < 3 as libc::c_int as libc::c_uint {
        ...
        pqcrystals_kyber768_ref_poly_add(r, r, &mut t);
        i = i.wrapping_add(1);
        i;
    }
    pqcrystals_kyber768_ref_poly_reduce(r);
...
unsafe extern "C" fn pack_pk(
    ...
    let mut i: size_t = 0;
    pqcrystals_kyber768_ref_polyvec_tobytes(r, pk);
    i = 0 as libc::c_int as size_t;
    while i < 32 as libc::c_int as size_t {
        *r
            .offset(
                i.wrapping_add((3 as libc::c_int * 384 as libc::c_int) as size_t)
                    as isize,
...
unsafe extern "C" fn unpack_pk(
    ...
    let mut i: size_t = 0;
    pqcrystals_kyber768_ref_polyvec_frombytes(pk, packedpk);
    i = 0 as libc::c_int as size_t;
    while i < 32 as libc::c_int as size_t {
        *seed
            .offset(
...
unsafe extern "C" fn pack_sk(mut r: *mut uint8_t, mut sk: *mut polyvec) {
    pqcrystals_kyber768_ref_polyvec_tobytes(r, sk);
}
unsafe extern "C" fn unpack_sk(mut sk: *mut polyvec, mut packedsk: *const uint8_t) {
    pqcrystals_kyber768_ref_polyvec_frombytes(sk, packedsk);
}
unsafe extern "C" fn pack_ciphertext(
    ...
    pqcrystals_kyber768_ref_polyvec_compress(r, b);
...
unsafe extern "C" fn unpack_ciphertext(
    ...
    pqcrystals_kyber768_ref_polyvec_decompress(b, c);
...
pub unsafe extern "C" fn pqcrystals_kyber768_ref_indcpa_keypair_derand(
    ...
    let mut a: [polyvec; 3] = [polyvec {
        vec: [poly { coeffs: [0; 256] }; 3],
    }; 3];
    ...
    pqcrystals_kyber768_ref_gen_matrix(a.as_mut_ptr(), publicseed, 0 as libc::c_int);
    i = 0 as libc::c_int as libc::c_uint;
    while i < 3 as libc::c_int as libc::c_uint {
        let fresh3 = nonce;
    ...
    pqcrystals_kyber768_ref_polyvec_ntt(&mut skpv);
    pqcrystals_kyber768_ref_polyvec_ntt(&mut e);
    i = 0 as libc::c_int as libc::c_uint;
    while i < 3 as libc::c_int as libc::c_uint {
    ...
    pqcrystals_kyber768_ref_polyvec_add(&mut pkpv, &mut pkpv, &mut e);
    pqcrystals_kyber768_ref_polyvec_reduce(&mut pkpv);
    pack_sk(sk, &mut skpv);
...
pub unsafe extern "C" fn pqcrystals_kyber768_ref_indcpa_enc(
    ...
    pqcrystals_kyber768_ref_poly_frommsg(&mut k, m);
    ...
    let fresh7 = nonce;
    nonce = nonce.wrapping_add(1);
    pqcrystals_kyber768_ref_poly_getnoise_eta2(&mut epp, coins, fresh7);
    pqcrystals_kyber768_ref_polyvec_ntt(&mut sp);
    i = 0 as libc::c_int as libc::c_uint;
    while i < 3 as libc::c_int as libc::c_uint {
        pqcrystals_kyber768_ref_polyvec_basemul_acc_montgomery(
    ...
    pqcrystals_kyber768_ref_polyvec_basemul_acc_montgomery(&mut v, &mut pkpv, &mut sp);
    pqcrystals_kyber768_ref_polyvec_invntt_tomont(&mut b);
    pqcrystals_kyber768_ref_poly_invntt_tomont(&mut v);
    pqcrystals_kyber768_ref_polyvec_add(&mut b, &mut b, &mut ep);
    pqcrystals_kyber768_ref_poly_add(&mut v, &mut v, &mut epp);
    pqcrystals_kyber768_ref_poly_add(&mut v, &mut v, &mut k);
    pqcrystals_kyber768_ref_polyvec_reduce(&mut b);
...
pub unsafe extern "C" fn pqcrystals_kyber768_ref_indcpa_dec(
    ...
    pqcrystals_kyber768_ref_polyvec_ntt(&mut b);
    pqcrystals_kyber768_ref_polyvec_basemul_acc_montgomery(&mut mp, &mut skpv, &mut b);
    pqcrystals_kyber768_ref_poly_invntt_tomont(&mut mp);
    pqcrystals_kyber768_ref_poly_sub(&mut mp, &mut v, &mut mp);
...
pub unsafe extern "C" fn pqcrystals_kyber768_ref_ntt(mut r: *mut int16_t) {
    ...
    while len >= 2 as libc::c_int as libc::c_uint {
        ...
        while start < 256 as libc::c_int as libc::c_uint {
            let fresh8 = k;
            k = k.wrapping_add(1);
            zeta = pqcrystals_kyber768_ref_zetas[fresh8 as usize];
            j = start;
...
pub unsafe extern "C" fn pqcrystals_kyber768_ref_invntt(mut r: *mut int16_t) {
    ...
    while len <= 128 as libc::c_int as libc::c_uint {
        ...
        while start < 256 as libc::c_int as libc::c_uint {
            let fresh9 = k;
            k = k.wrapping_sub(1);
            zeta = pqcrystals_kyber768_ref_zetas[fresh9 as usize];
            j = start;
            while j < start.wrapping_add(len) {
...
pub unsafe extern "C" fn pqcrystals_kyber768_ref_keypair_derand(
    ...
    pqcrystals_kyber768_ref_indcpa_keypair_derand(pk, sk, coins);
    memcpy(
...
pub unsafe extern "C" fn pqcrystals_kyber768_ref_dec(
    ...
    let mut buf: [uint8_t; 64] = [0; 64];
    let mut kr: [uint8_t; 64] = [0; 64];
    let mut cmp: [uint8_t; 1088] = [0; 1088];
    let mut pk: *const uint8_t = sk
        .offset((3 as libc::c_int * 384 as libc::c_int) as isize);
    pqcrystals_kyber768_ref_indcpa_dec(buf.as_mut_ptr(), ct, sk);
    i = 0 as libc::c_int as size_t;
    while i < 32 as libc::c_int as size_t {
    ...
    fail = pqcrystals_kyber768_ref_verify(
        ct,
        cmp.as_mut_ptr(),
        (3 as libc::c_int * 320 as libc::c_int + 128 as libc::c_int) as size_t,
    );
